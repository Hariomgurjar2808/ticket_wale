...
import BusSkeleton from "../../components/BusSkeleton";
...
const normalizeBus = (bus, index = 0) => ({
  ...(() => {
    ...
    return {
      ...
      operator: pickPreferredString(bus.operator, base.operator || "TicketWale Partner", [
        "TicketWale Partner",
      ]),
...
function BookingPageContent() {
  ...
  const bookedSeatSet = useMemo(() => new Set(selectedBus?.bookedSeats || []), [selectedBus]);
        ...
        >
          <Box
            sx={{
              ...
              borderRadius: "999px",
              background:
                "radial-gradient(circle, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0.01) 70%)",
            }}
          />
          <Box
            sx={{
              ...
              borderRadius: "999px",
              background:
                "radial-gradient(circle, rgba(255,255,255,0.16) 0%, rgba(255,255,255,0.01) 70%)",
            }}
          />
          ...
          <Typography
            variant="h3"
            sx={{ fontWeight: 900, mt: 1, mb: 1.5, fontSize: { xs: "2rem", md: "2.6rem" } }}
          >
            {from} to {to}
          </Typography>
          ...
          <Box className="flex flex-wrap gap-2">
            <Chip
              label={`${buses.length} Buses Available`}
                ...
                <Box className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ color: "#4b5563" }}>
                      Rating: <strong>{bus.rating}</strong> | Seats Left:{" "}
                      <strong>{getAvailableSeats(bus)}</strong>
                    </Typography>
                  </Box>
        ...
        <Box className="mt-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:p-6">
          ...
          {selectedBus && (
            ...
            >
              ...
              <Box className="mb-1 flex flex-wrap gap-2">
                <Chip
                  label={`Bus Type: ${selectedBus.busType}`}
                  size="small"
                  sx={{ backgroundColor: "#e0e7ff", color: "#312e81", fontWeight: 700 }}
                />
                <Chip
                  label={`Duration: ${selectedBus.duration}`}
                  size="small"
                  sx={{ backgroundColor: "#ffedd5", color: "#9a3412", fontWeight: 700 }}
                />
                <Chip
                  label={`Fare: Rs ${selectedBus.price}/seat`}
                  size="small"
          ...
          <Box className="mb-3 flex flex-wrap gap-2">
            <Chip
              label="Available"
              size="small"
              sx={{ backgroundColor: "#dbeafe", color: "#1e40af", fontWeight: 700 }}
            />
            <Chip
              label="Selected"
              size="small"
              sx={{ backgroundColor: "#ede9fe", color: "#5b21b6", fontWeight: 700 }}
            />
            <Chip
              label="Booked"
              size="small"
              sx={{ backgroundColor: "#fecaca", color: "#991b1b", fontWeight: 700 }}
            />
          </Box>
                  ...
                  {row.map((item) => {
                    ...
                    if (item.type === "empty") {
                      return (
                        <Box
                          key={item.key}
                          sx={{ width: isSleeperType(selectedBus?.busType) ? 54 : 44 }}
                        />
                      );
                    }
